               ...
               build_version_command:(struct build_version_command const *)build_version_command
{
    ...
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                           ...
                           :[@(build_version_command->ntools) stringValue]];

    for (uint32_t i = 0; i < build_version_command->ntools; i++) {
        [node.details setAttributes:MVUnderlineAttributeName,@"YES",nil];

        uint32 tool = [dataController read_uint32:range lastReadHex:&lastReadHex];
        NSString *toolString = nil;
        switch (tool) {
            case TOOL_CLANG: {
                toolString = @"CLANG";
            } break;
        ...
        uint32 toolVersion = [dataController read_uint32:range lastReadHex:&lastReadHex];
        NSString *toolVersionString = [NSString stringWithFormat:@"%u.%u.%u",
                                       (toolVersion >> 16),
                                       ((toolVersion >> 8) & 0xff),
                                       (toolVersion & 0xff)];
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
  ...
  switch (command)
  {
    case LC_SEGMENT:
    {
      ...
      for (uint32_t nsect = 0; nsect < segment_command->nsects; ++nsect)
      {
        ...
        sectionInfo[section->addr] = make_pair(section->offset + imageOffset, (void *)section);
    ...
    case LC_SEGMENT_64:
    {
      ...
      for (uint32_t nsect = 0; nsect < segment_command_64->nsects; ++nsect)
      {
        ...
        // preserv section fileOffset/sectName for RVA lookup
        sectionInfo[section_64->addr] = make_pair(section_64->offset + imageOffset, (void *)section_64);

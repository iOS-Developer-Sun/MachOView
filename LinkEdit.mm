...
@interface MachOLayoutFixupImport : NSObject
...
@implementation MachOLayoutFixupImport
        ...
        if ((nlist_64->n_type & N_TYPE) == N_SECT)
        {
            ...
            if ((nlist_64->n_type & N_STAB) == 0)
            {
                ...
                nameToStore = (nameToStore != nil 
                               ? [nameToStore stringByAppendingFormat:@"(%@)", symbolName] 
                               : [NSString stringWithFormat:@"0x%qX (%@)", nlist_64->n_value, symbolName]);
                symbolsMap[symbolName] = @(nlist_64->n_value);
                [symbolNames setObject:nameToStore
                              ...
                              length:(uint32_t)length
{
    ...
    char *symbolsPool = NULL;
    if (dyld_chained_fixups_header->symbols_format == 0) {
        symbolsPool = (char *)[self imageAt:location + dyld_chained_fixups_header->symbols_offset];
    }

    if (symbolsPool) {
        [self createChainedFixupImports:parent caption:@"Imports" location:location + dyld_chained_fixups_header->imports_offset length:dyld_chained_fixups_header->symbols_offset - dyld_chained_fixups_header->starts_offset header:dyld_chained_fixups_header symbolsPool:symbolsPool];
        [self createChainedFixupSymbols:parent caption:@"Symbols" location:location + dyld_chained_fixups_header->symbols_offset length:length - dyld_chained_fixups_header->symbols_offset header:dyld_chained_fixups_header symbolsPool:symbolsPool];
    }

    MATCH_STRUCT(dyld_chained_starts_in_image, location + dyld_chained_fixups_header->starts_offset)
    [self createChainedFixupSegmentsNode:parent caption:@"Fixedup Header" location:location + dyld_chained_fixups_header->starts_offset length:sizeof(uint32_t) * (dyld_chained_starts_in_image->seg_count + 1)];


    return node;
}

- (MVNode *)createChainedFixupSegmentsNode:parent
                                   caption:(NSString *)caption
                                  location:(uint32_t)location
                                    length:(uint32_t)length
{
    ...
    MATCH_STRUCT(dyld_chained_starts_in_image, NSMaxRange(range))
    [dataController read_uint32:range lastReadHex:&lastReadHex];
    [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                           :lastReadHex
                           :@"Fixups Segments Count"
                           :[NSString stringWithFormat:@"%u", dyld_chained_starts_in_image->seg_count]
    ];
    
    for (NSInteger i = 0; i < dyld_chained_starts_in_image->seg_count; i++) {
        uint32_t offset = [dataController read_uint32:range lastReadHex:&lastReadHex];
        const struct segment_command_64 *segment_64 = [self getSegment64ByIndex:i];
        NSString *segmentName = NSSTRING(string(segment_64->segname,16).c_str());
        [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                               :lastReadHex
                               :[NSString stringWithFormat:@"Segment %@ Fixup", segmentName]
                               :(offset == 0) ? @"No Fixup" : [NSString stringWithFormat:@"%.8lX", location + offset]
        ];
        
        if (offset) {
            MVNodeSaver nodeSaver;
            NSRange range = NSMakeRange(location + offset, 0);
            MATCH_STRUCT(dyld_chained_starts_in_segment, NSMaxRange(range));
            MVNode * node = [parent insertChildWithDetails:[NSString stringWithFormat:@"%@ Fixups", segmentName] location:location + offset length:dyld_chained_starts_in_segment->size saver:nodeSaver];
            
            [dataController read_uint32:range lastReadHex:&lastReadHex];
            [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                                   :lastReadHex
                                   :@"Size"
                                   :[NSString stringWithFormat:@"%u", dyld_chained_starts_in_segment->size]];
            
            [dataController read_uint16:range lastReadHex:&lastReadHex];
            [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                                   :lastReadHex
                                   :@"Page Size"
                                   :[NSString stringWithFormat:@"%u", dyld_chained_starts_in_segment->page_size]];
            
            [dataController read_uint16:range lastReadHex:&lastReadHex];
            [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                                   :lastReadHex
                                   :@"Pointer Format"
                                   :[NSString stringWithFormat:@"%u", dyld_chained_starts_in_segment->pointer_format]];
            
            [dataController read_uint64:range lastReadHex:&lastReadHex];
            [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                                   :lastReadHex
                                   :@"Segment Offset"
                                   :[NSString stringWithFormat:@"%u", dyld_chained_starts_in_segment->segment_offset]];
            
            [dataController read_uint32:range lastReadHex:&lastReadHex];
            [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                                   :lastReadHex
                                   :@"Max Valid Pointerize"
                                   :[NSString stringWithFormat:@"%u", dyld_chained_starts_in_segment->max_valid_pointer]];
            
            [dataController read_uint16:range lastReadHex:&lastReadHex];
            [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                                   :lastReadHex
                                   :@"Page Count"
                                   :[NSString stringWithFormat:@"%u", dyld_chained_starts_in_segment->page_count]];

            [node.details setAttributes:MVUnderlineAttributeName,@"YES",nil];

            struct segment_command_64 const * segment_64 = [self getSegment64ByIndex:i];
            for (NSInteger j = 0; j < dyld_chained_starts_in_segment->page_count; j++) {
                [dataController read_uint16:range lastReadHex:&lastReadHex];
                uint16_t firstOffset = dyld_chained_starts_in_segment->page_start[j];
                [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                                       :lastReadHex
                                       :[NSString stringWithFormat:@"Page %d First Offset", j]
                                       :[NSString stringWithFormat:@"%u", firstOffset]];
                // fixup pages
                if (!segment_64 || firstOffset == DYLD_CHAINED_PTR_START_NONE) {
                    continue;
                }
                
                uint32_t page = segment_64->fileoff + dyld_chained_starts_in_segment->page_size * j + firstOffset;
                [self fixupPage:page segInfo:(struct dyld_chained_starts_in_segment *)dyld_chained_starts_in_segment];
            }
        }
    }
...
//#endif
        ...
        case DYLD_CHAINED_PTR_64_OFFSET:
            [self fixupPage64:pageOffset segInfo:segInfo offsetBased:true];
            break;
...
- (MVNode *)createChainedFixupImports:parent
                                   caption:(NSString *)caption
                                  location:(uint32_t)location
                                    length:(uint32_t)length
                               header:(const struct dyld_chained_fixups_header *)header
                          symbolsPool:(char *)symbolsPool
{
    MVNodeSaver nodeSaver;
    MVNode * node = [parent insertChildWithDetails:caption location:location length:length saver:nodeSaver];
    ...
    switch (header->imports_format) {
        case DYLD_CHAINED_IMPORT: {
            struct dyld_chained_import *imports = (struct dyld_chained_import*)((uint8_t*)header + header->imports_offset);
            for (uint32_t i=0; i < header->imports_count; ++i) {
                ...
                uint32_t item = [dataController read_uint32:range lastReadHex:&lastReadHex];
                ...
                [node.details appendRow:[NSString stringWithFormat:@"%.8lX", range.location]
                                       :lastReadHex
                                       :@"Import"
                                       :[NSString stringWithFormat:@"%.8lX", item]];
        ...
        case DYLD_CHAINED_IMPORT_ADDEND: {
            struct dyld_chained_import_addend *imports = (struct dyld_chained_import_addend *)((uint8_t*)header + header->imports_offset);
            for (uint32_t i=0; i < header->imports_count; ++i) {
                const char* symbolName = &symbolsPool[imports[i].name_offset];
                uint8_t libVal = imports[i].lib_ordinal;
                if ( libVal > 0xF0 )
                ...
                uint32_t item = [dataController read_uint32:range lastReadHex:&lastReadHex];
                int32_t appendex = [dataController read_int32:range lastReadHex:&lastReadHex];
        ...
        case DYLD_CHAINED_IMPORT_ADDEND64: {
            struct dyld_chained_import_addend64 *imports = (struct dyld_chained_import_addend64 *)((uint8_t*)header + header->imports_offset);
            for (uint32_t i=0; i < header->imports_count; ++i) {
                const char* symbolName = &symbolsPool[imports[i].name_offset];
                uint16_t libVal = imports[i].lib_ordinal;
                if ( libVal > 0xFFF0 )
                    libOrdinal = (int16_t)libVal;
                else
                    libOrdinal = libVal;

                uint64_t item = [dataController read_uint64:range lastReadHex:&lastReadHex];
                uint64_t appendex = [dataController read_uint64:range lastReadHex:&lastReadHex];
...
- (MVNode *)createChainedFixupSymbols:parent
                              caption:(NSString *)caption
                             location:(uint32_t)location
                               length:(uint32_t)length
                               header:(const struct dyld_chained_fixups_header *)header
